    ...
    for row in table.rows:
        if post_vuln_row:
            ...
            all_sst_updates.append([
                row.cells[1].text,
                kb_or_build,
                next((a for a, b in release_versions if b == table_name), None),
...
import io as _io
df_qual = pd.DataFrame(all_sst_updates, columns=qual_columns)
_csv_buf = _io.StringIO()
df_qual.to_csv(_csv_buf, index=False)
qual_registry_csv_output = _csv_buf.getvalue()
`;
